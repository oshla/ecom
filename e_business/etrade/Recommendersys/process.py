...
def getUsername(var):
    ...
    t_list = []
    with open('D:\ecom\e_business\etrade\Recommendersys\mentdf.csv', errors="replace") as csvfile:    #Reads the csv that contains the usernames and their respective tweets
    ...
    for line in g_trg:
        if len(line) > 0 and line[0] == '':  # removes blank entries from lines, mentioning at least one name
            del line[0]
    ...
    for items in list1:
        for items1 in list1:
            if (items1 != items and items1!='') and (nx.has_path(G,items, items1)):
    ...
    ##Normalize
    can=pd.read_csv('D:\ecom\e_business\etrade\Recommendersys\samp_data.csv') #read the csv file that contains the user registration entries
    username=can["sc_handle"] #store only the twitter username column in the variable "username"
    #print(can)
    ...
    can_age_normalized.set_index('sc_handle', inplace=True) #set the twitterusername as index
